...
export default function DatabaseNode({ data }: NodeProps) {
  ...
  return (
    <>
      ...
      <div className="bg-gray-900 border border-cyan-700 rounded-lg p-3 min-w-[160px] max-w-[220px] shadow-lg">
        <div className="flex items-center gap-2 mb-1">
          <span className="text-xs bg-cyan-900 text-cyan-300 px-1.5 py-0.5 rounded font-mono">DB</span>
          {/* Cylinder icon */}
          <svg className="w-3.5 h-3.5 text-cyan-500 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
        ...
        {d.technology && (
          <p className="text-xs text-cyan-400 truncate">{d.technology}</p>
        )}
        {d.description && (
